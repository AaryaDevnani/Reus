...
function Recipes() {
  ...
  const [searchFinal, setSearchFinal] = useState('Chicken');
  const [recipes, setRecipes] = useState([]);
  const [byproducts, setByproducts] = useState([]);
  const [ingredients, setIngredients] = useState([]);
  const [adding, setAdding] = useState(['Chicken']);
  ...
  const getAdding = (name) => {
    if (adding.includes(name)) {
      ...
      setSearchFinal(adding.filter((a) => a !== name).join(' '));
      setSearch(adding.filter((a) => a !== name).join(' '));
    } else {
      setAdding([...adding, name]);
      setSearchFinal(searchFinal + ' ' + name);
      setSearch(searchFinal + ' ' + name);
    }
  };
            ...
            <div className="btns">
              {ingredients.map((ingredient) => (
                <input
                  ...
                  type="button"
                  className={
                    adding.includes(ingredient.name)
                      ? 'butn pill pill-active'
                      : 'butn pill pill-inactive'
                  }
                  value={ingredient.name}
                  onClick={() => {
                    getAdding(ingredient.name);
                  }}
                />
              ))}
            </div>
          ...
          {recipes.map(({ recipe }) => (
            <div className="item">
              <p
                ...
                dangerouslySetInnerHTML={{ __html: recipe.label }}
              ></p>
